...
sanoid & syncoid now support prometheus out of the box.  
Since we're running as deamons, both tools produce `.prom` files which can be picked up by node_exporter's textcollector plugin.

To enable prometheus metric support, add `--prometheus-textfile=/path/to/your/promfile` to your invocation.
You can additionnaly use `--identifier=some_string` as optional labels for both invocations. 
Example:
```
sanoid --take-snapshots - --prometheus-textfile=/var/lib/node_exporter/textfile_collector/sanoid_take.prom
sanoid --prune-snapshots --verbose --prometheus-textfile=/var/lib/node_exporter/textfile_collector/sanoid_prune.prom
syncoid --identifier prod --prometheus-textfile=/var/lib/node_exporter/textfile_collector/syncoid-public_vm1.prom pool/tank pool2/tank2
```

An example grafana dashboard can be found in the examples directory

![image](examples/grafana-dashboard_0.png)
![image](examples/grafana(dashboard_1.png)

The following metrics are generated by sanoid:

- sanoid_run_timestamp_seconds
- sanoid_run_duration_seconds
- sanoid_run_success
- sanoid_snapshot_count
- sanoid_snapshots_taken_total
- sanoid_snapshots_pruned_total

The following metrics are generated by syncoid:

- syncoid_run_timestamp_seconds
- syncoid_run_duration_seconds
- syncoid_run_success
- syncoid_dataset_sync_timestamp_seconds
- syncoid_dataset_sync_duration_seconds
- syncoid_dataset_sync_success
- syncoid_dataset_target_snapshot_count
- syncoid_dataset_bytes_sent_estimated_total
- syncoid_dataset_throughput_bytes_per_second